...
export default function CustomersPage() {
  const { getCustomers, deleteThirdParty } = useInvoicingStore();
  ...
  const handleDelete = (thirdParty: ThirdParty) => {
    if (confirm(`Êtes-vous sûr de vouloir supprimer le client "${thirdParty.name}" ?`)) {
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-claude-text">Clients</h1>
          <p className="text-claude-text-muted mt-2">
            Gestion de vos clients et contacts commerciaux
          </p>
        </div>
        {!showForm && (
      ...
      {showForm ? (
        <Card>
          <CardHeader>
            <CardTitle>
              {editingThirdParty ? 'Modifier le client' : 'Nouveau client'}
            </CardTitle>
            <CardDescription>
              {editingThirdParty
        ...
        <Card className="bg-blue-50 border-blue-200">
          <div className="p-6">
            ...
            <p className="text-blue-800 mb-4">
              Les clients vous permettent de générer des factures, des devis et de suivre vos
              encours. Les informations essentielles à renseigner sont :
            </p>
            <ul className="list-disc list-inside text-blue-800 space-y-1 mb-4">
              <li>Raison sociale</li>
              <li>ICE (obligatoire pour les factures B2B)</li>
