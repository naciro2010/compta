  ...
  return {
    ...
    saveEmployee(){
      if (!this.employeeForm.nom) {
        alert(this.translate('payroll.validation.name'))
        return
      }
      if (this.employeeForm.matriculeCnss && !/^[a-zA-Z0-9-]+$/.test(this.employeeForm.matriculeCnss)) {
        alert(this.translate('payroll.validation.matricule'))
        return
      }
      if (Number(this.employeeForm.salaireBase) < 0) {
        alert(this.translate('payroll.validation.salary'))
        return
      }
      const payload = {
        ...this.employeeForm,
    ...
    generate(){
      if (!this.employees.length) {
        alert(this.translate('payroll.validation.needEmployee'))
        return
      }
...
export function renderPayroll(){
  ...
  <section class="space-y-8" x-data="payrollView()">
    ...
    <div x-show="showEmployeeModal" class="modal" x-cloak @keydown.escape.window="showEmployeeModal=false">
      <div class="modal__overlay" @click="showEmployeeModal=false"></div>
      <div class="modal__panel" role="dialog" aria-modal="true">
        <div class="modal__header">
          <h2 class="modal__title" x-text="editingId ? $store.i18n.t('payroll.employees.modal.editTitle') : $store.i18n.t('payroll.employees.modal.createTitle')"></h2>
          <button class="modal__close" @click="showEmployeeModal=false">×</button>
        </div>
